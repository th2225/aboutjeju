...
<body class="food">
	...
	<div class="row mt-5 justify-content-evenly">
		...
		ArrayList<PlaceVO> list = dao.selectAll(startNo, category);
			...
			<div class="card mb-3" style="width:540px;">
				<div class="row g-0">
					<div class="col-4 h-120">
						<img src="<%=vo.getPimage()%>" class="rounded-start w-100" alt="<%=vo.getPname()%>">
					</div>
					<div class="col-8">
						<div class="card-body">
							<h5 class="card-title"><%=vo.getPname()%></h5>
							<div class="card-text"><%=vo.getPphone()%></div>
							<div class="card-text"><%=vo.getPaddress()%></div>
						</div>
					</div>
				</div>
			</div>
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination justify-content-center mt-5">
			...
			<li class="page-item"><a class="page-link" href="place.jsp?category=<%=cat%>&cp=<%=(currentPage - 5 < 1) ? 1 : (currentPage - 5)%>"
			...
			<li class="page-item"><a class="page-link bg-primary text-white" href="place.jsp?category=<%=cat%>&cp=<%=i%>"><%=i%></a></li>

			<%
			} else {
			%>

			<li class="page-item"><a class="page-link" href="place.jsp?category=<%=cat%>&cp=<%=i%>"><%=i%></a></li>
			...
			<li class="page-item"><a class="page-link"
				href="place.jsp?category=<%=cat%>&cp=<%=(currentPage + 5 > totalPage) ? (totalPage) : (currentPage + 5)%>" aria-label="Next" title="5페이지 뒤로">
